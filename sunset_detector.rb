...
class SunsetDetector
  ...
  def should_tweet_now?(most_recent_photo)
    num_sunsets = self.previous_sunsets
    puts "fancypants math says this is " + FancyPantsMath::do_some_calculus(self.previous_sunsets[-num_sunsets..-1]).map(&:sunsettiness).compact ? "" : "not " + "a sunset" if self.previous_sunsets && self.previous_sunsets[-num_sunsets..-1]
